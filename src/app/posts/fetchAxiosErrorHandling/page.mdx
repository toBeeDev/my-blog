...
- 기존
  ...
  ````tsx
  const handleServerInternalError = (response: AxiosResponse<ErrorResData>, error: AxiosError<ErrorResData>) => {
  const message = response.status != serviceUnavailableStatusCode ? SERVER_ERROR_MSG : TEMP_SERVER_ERROR_MSG
  const serviceInternalError = new ServerInternalError(message, error.response, error.request)
